...
/**
 * Loads and displays Njiandan view themes. Can also handle output of some binary
 * files, such as image, Javascript, and CSS files.
 */
class ThemeView_Core {
	...
	public function set_filename($name, $type = NULL)
	{
		...
		if ($type == NULL)
		{
			...
			$this->kohana_filename = Kohana::find_file('themes/' . $theme, $name, TRUE);
		...
		else
		{
			...
			// Load the filename and set the content type
			$this->kohana_filename = Kohana::find_file('themes/' . $theme, $name, TRUE, $type);
	...
	public function is_set( $key = FALSE )
	{
		...
		if (is_array($key))
		{
			...
			foreach ($key as $property)
			{
				...
				$result[$property] = (array_key_exists($property, $this->kohana_local_data) OR array_key_exists($property, ThemeView::$kohana_global_data)) ? TRUE : FALSE;
			}
		}
		else
		{
			// Otherwise just check one property
			$result = (array_key_exists($key, $this->kohana_local_data) OR array_key_exists($key, ThemeView::$kohana_global_data)) ? TRUE : FALSE;
		}
	...
	public static function set_global($name, $value = NULL)
	{
		if (is_array($name))
		{
			foreach ($name as $key => $value)
			{
				ThemeView::$kohana_global_data[$key] = $value;
			}
		}
		else
		{
			ThemeView::$kohana_global_data[$name] = $value;
		}
	}
	...
	public function &__get($key)
	{
		...
		if (isset(ThemeView::$kohana_global_data[$key]))
			return ThemeView::$kohana_global_data[$key];
	...
	public function render($print = FALSE, $renderer = FALSE)
	{
		...
		if (is_string($this->kohana_filetype))
		{
			...
			$data = array_merge(ThemeView::$kohana_global_data, $this->kohana_local_data);
            ...
            // change the themes meta uri
            $output = ThemeView::meta_filter($output);
			if ($print === TRUE)
			{
				// Display the output
				echo $output;
    ...
    /**
     * replace the themes (image, css, swf, js, ) uri
     *
     * @param   string    the render content
     * @return  string    the content
     */
     public static function meta_filter($content) {
